...
serve(async (req) => {
  ...
  try {
    ...
    try {
      const { data, error } = await supabase.auth.admin.getUserByEmail('admin@example.com');
      if (data?.user) {
        adminExists = true;
        console.log("Admin user already exists:", data.user.id);
      }
      if (error) {
    ...
    if (!adminExists) {
      ...
      try {
        ...
        console.error("Error during admin user creation:", creationError);
        return new Response(
          JSON.stringify({ 
