...
-- Fix care team room creation to ensure doctors are always added as members
...
-- Delete any duplicate room members to clean up potential issues
CREATE OR REPLACE FUNCTION public.clean_duplicate_room_members()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  -- Remove duplicate members, keeping the oldest membership for each unique combo
  WITH duplicates AS (
    SELECT id,
           row_number() OVER (
...
-- Execute the cleanup
SELECT public.clean_duplicate_room_members();
