...
-- Update all room messages functions to have more consistent and higher default limits
CREATE OR REPLACE FUNCTION public.get_room_messages_with_role(
  p_room_id UUID,
  p_limit INTEGER DEFAULT 500,  -- Ensure consistent limit with frontend
...
BEGIN
  ...
  -- IMPORTANT: Using ORDER BY created_at DESC to get newest messages first
  RETURN QUERY
  SELECT 
    rm.id,
    rm.sender_id,
  ...
  ORDER BY rm.created_at DESC -- Using DESC to get newest messages first
...
-- Also update any related functions with direct limit constants
-- For example, if there's a get_room_messages function without role:
CREATE OR REPLACE FUNCTION public.get_room_messages(
  p_room_id UUID,
  p_limit INTEGER DEFAULT 500, -- Ensure consistent high limit
  p_offset INTEGER DEFAULT 0
)
RETURNS TABLE (
  id UUID,
...
BEGIN
  ...
  FROM room_messages rm
  WHERE rm.room_id = p_room_id
  ORDER BY rm.created_at DESC -- Newest first
  LIMIT p_limit OFFSET p_offset;
END;
$$;
